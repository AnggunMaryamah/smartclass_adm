...
.stat-card {
    ...
    position: relative;
    overflow: hidden;
    border-left: 6px solid transparent; /* default, akan diberi warna */
}

/* Warna border kiri per kartu */
.stat-card.border-blue { border-left-color: #0EA5E9; }
.stat-card.border-green { border-left-color: #10B981; }
.stat-card.border-cyan { border-left-color: #06B6D4; }
...
.orders-table tbody tr:nth-child(even) { background: #F8FAFC; }
.orders-table tbody tr:hover { background: #EEF6FF; }
    ...
    <div class="card-table-wrapper">
        <div class="card-table-header">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                ...
                <select id="filterJenjang" onchange="applyFilterJenjang()" style="padding:6px 12px; border-radius:8px; border:1px solid #ccc;">
                ...
                <tr>
                    ...
                    <td>
                        @php
                            ...
                            if (!in_array($jenjang,$validJenjang)) { $jenjang = null; }

                            $bgJenjang = '#E3F2FD'; $colorJenjang='#1976D2';
                            if($jenjang==='SMP') { $bgJenjang='#E8F5E9'; $colorJenjang='#388E3C'; }
                            elseif($jenjang==='SMA') { $bgJenjang='#FFF3E0'; $colorJenjang='#F57C00'; }
                        @endphp
                        @if($jenjang)
                            <span class="badge-jenjang" style="background:{{ $bgJenjang }};color:{{ $colorJenjang }};">
                                {{ $jenjang }}
                            </span>
                        @endif
                    </td>
                    <td>
                        ...
                        @endphp
                        <span class="badge-status" style="background:{{ $bgStatus }};color:{{ $colorStatus }};">
