...
class GuruController extends Controller
{
    ...
    public function kelasStore(Request $request)
    {
        ...
        // Jika durasi = custom, ambil dari durasi_custom
        if ($validated['durasi'] === 'custom' && ! empty($validated['durasi_custom'])) {
            $validated['durasi'] = $validated['durasi_custom'];
        }

        // Hapus durasi_custom karena tidak ada di database
        unset($validated['durasi_custom']);

        // Set field tambahan
        $validated['guru_id'] = $guru->id;
        $validated['status'] = 'aktif';
        $validated['jumlah_siswa'] = 0;
    ...
    public function kelasUpdate(Request $request, $id)
{
    ...
    public function kelasDetail($id)
    {
        ...
        return view('guru.kelas.show', compact('kelas', 'daftarSiswa'));
    }
    ...
    public function laporan()
    {
        $guru = Guru::first();
        if (! $guru) {
            return 'Tidak ada data guru. Silakan tambahkan minimal 1 guru di database.';
        }
